...
int parse_ipv4(const uint8_t *buffer, size_t length, IPv4Header *header){
	...
	if (buffer == NULL || header == NULL || length < 20) {
		log_error("Invalid input to parse_ipv4");
    		return -1;
	}
	...
	if (header->version != 4) {
		log_error("Invalid IPv4 version");
 		return -1;
	}
	...
	if (header_length > length) {
		log_error("IPv4 header length exceeds buffer length");
		return -1;
	}

	header->total_length = ((uint16_t)buffer[2] << 8) | buffer[3];
	if (header->total_length > length) {
		log_error("IPv4 total length exceeds buffer length");
		return -1;
	}

	if (header->total_length < header_length) {
		log_error("IPv4 total length is less than header length");
		return -1;
	}
